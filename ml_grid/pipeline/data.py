...
class pipe():
    ...
    def __init__(self, file_name, drop_term_list, local_param_dict, base_project_dir, param_space_index, additional_naming = None, test_sample_n=0): # kwargs**
        ...
        log_folder(local_param_dict =local_param_dict,
        ...
        self.pertubation_columns, self.drop_list = get_pertubation_columns(all_df_columns = self.all_df_columns,
        ...
        print(f"Using {len(self.pertubation_columns)}/{len(self.all_df_columns)} columns for {self.outcome_variable} outcome")
        ...
        self.drop_list = handle_correlation_matrix(local_param_dict = local_param_dict,
                                                   drop_list = self.drop_list,
                                                   df = self.df)

        self.drop_list = handle_percent_missing(local_param_dict = local_param_dict, 
        ...
        final_column_list = [self.X for self.X in self.pertubation_columns if (self.X not in self.drop_list and self.X in self.df.columns )]
        ...
        if(self.verbose >=1):
            print(f"len final droplist: {len(self.drop_list)} \ {len(list(self.df.columns))}")
            #print('\n'.join(map(str, self.drop_list[0:5])))

        print("------------------------")
        
        self.X_train, self.X_test, self.y_train, self.y_test, self.X_test_orig, self.y_test_orig = get_data_split(X = self.X,
                                                                                                                y = self.y,
        ...
        if(target_n_features != 100):
            ...
            target_n_features_eval = int((target_n_features/100) * len(self.X_train.columns))
            
            if(target_n_features_eval < len(self.X_train.columns)):
                target_n_features_eval = len(self.X_train.columns)
            
            print(f"Pre target_n_features {target_n_features}% reduction {target_n_features_eval}/{len(self.X_train.columns)}")
            try:
                
                self.X_train, self.X_test, self.X_test_orig = feature_importance_methods.handle_feature_importance_methods(
                                                                                                self,
                ...
                print("failed to feature methods", e)
