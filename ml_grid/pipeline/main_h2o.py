...
# from ml_grid.model_classes.h2o_class import \
#     h2o_class

from ml_grid.model_classes.h2o_classifier_class import h2o_classifier_class

from ml_grid.pipeline import h2o_evaluate
...
#                                      parameter_space_size=self.parameter_space_size),
            ...
            #h2o_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train)

#             knn_classifiers_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                                   parameter_space_size=self.parameter_space_size),

#             quadratic_discriminant_analysis_class(
#                 X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train, parameter_space_size=self.parameter_space_size),

#             SVC_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                       parameter_space_size=self.parameter_space_size),

#             XGB_class_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                             parameter_space_size=self.parameter_space_size),

#             mlp_classifier_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                                  parameter_space_size=self.parameter_space_size),
...
#             kerasClassifier_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                                   parameter_space_size=self.parameter_space_size),

#             GaussianNB_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                              parameter_space_size=self.parameter_space_size),

#             adaboost_class(X=self.ml_grid_object.X_train, y=self.ml_grid_object.y_train,
#                            parameter_space_size=self.parameter_space_size),
...
#                             print("What is this?")
#                             print(f"{elem.method_name, param} {type(elem.parameter_space.get(param))}")
...
#                     pass
        ...
        self.mean_parameter_space_val = None #np.mean(self.pg_list)
                                
        self.sub_sample_parameter_val = None #int(self.sub_sample_param_space_pct * self.mean_parameter_space_val)              
    ...
    def execute(self):
        ...
        if self.multiprocess:
            ...
            for k in range(0, len(self.arg_list)):
                try:
                    print("evaluating h2o searching...")
                    h2o_evaluate.h2o_evaluate_class(
                        *self.arg_list[k]
                    ...
                    if(self.error_raise):
                        input("error thrown in grid_search_crossvalidate on model class list")

        print(f"Model error list: nb. errors returned from func: {self.model_error_list}")
